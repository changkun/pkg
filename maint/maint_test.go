...
// maint.Call is either executed on the main thread or not.
	...
	for i := 0; i < ncalls; i++ {
		...
		go func() {
			...
			maint.Call(func() {
				...
				t.Logf("call is not executed on the main thread, want %d, got %d", initTid, current)
			})
		}()
		go func() {
			defer wg.Done()
